...
import { queryTeachersApi } from '@/api/teacher'
...
const submitQuery = async () => {
  ...
  try {
    const result = await queryTeachersApi({
      field: field.value,
  ...
  <div class="directory-page">
    ...
    <div class="result-card">
      <el-table v-loading="loading" :data="rows" empty-text="暂无数据">
        ...
        <el-table-column prop="teacherNo" label="教号" min-width="160" />
        <el-table-column prop="gender" label="性别" min-width="90">
          <template #default="{ row }">
            {{ row.gender === 1 ? '男' : row.gender === 2 ? '女' : '-' }}
          </template>
        </el-table-column>
        <el-table-column prop="title" label="职称" min-width="180" />
      </el-table>

      <el-empty
        v-if="searched && !loading && rows.length === 0"
        class="empty-state"
        description="未查询到符合条件的教工信息"
      />
    </div>
  </div>
</template>
